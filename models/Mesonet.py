...
class MesoInception4(nn.Module):
	...
	def __init__(self, num_classes=2):
		...
		self.dropout = nn.Dropout2d(0.5)
		self.fc1 = nn.Linear(16*7*7, 16) # 16*4*4
		self.fc2 = nn.Linear(16, num_classes)
	...
	def forward(self, input):
		x = self.InceptionLayer1(input) #(Batch, 11, 64, 64)
		x = self.InceptionLayer2(x) #(Batch, 12, 32, 32)

		x = self.conv1(x) #(Batch, 16, 32 ,32)
		x = self.relu(x)
		x = self.bn1(x)
		x = self.maxpooling1(x) #(Batch, 16, 16, 16)

		x = self.conv2(x) #(Batch, 16,16, 16)
		x = self.relu(x)
		x = self.bn1(x)
		x = self.maxpooling2(x) #(Batch, 16, 4, 4)
